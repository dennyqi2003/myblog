...
[^Author]: DennyQi
[^Title]: 监督学习
[^Tag]: Informatics, Machine Learning, Machine Learning Foundations, tmp
...
从数学上，如果我们设所有样本（图片）的集合为$\mathscr{X}$，标签集合为$\mathscr{Y}$，那么ground truth就是样本集$\mathscr{X}\times \mathscr{Y}$上的一个概率空间：

$$(\mathscr{X}\times \mathscr{Y},\sigma(\mathscr{X}\times\mathscr{Y}),\mathbb{P})$$
...
根据训练集，我们要训练出用于预测的函数$f:\mathscr{X}\to \mathscr{Y}$，称为预测函数(prediction function)。我们通常把$f$的选择分为两个过程：第一步，选定一个基本的模型，这规定了一个$\mathscr{X}\to \mathscr{Y}$的函数类$\mathscr{F}$；第二步，调整模型的参数，也即找到$\mathscr{F}$中的某个“较好”的函数。
...
有了loss function以后，我们自然可以这样建模来评估$\mathscr{X}$上$h$的表现：考察从ground truth中采样(sample)的图像的正确标签和$h$预测的标签的loss的期望：$L(h)=\mathbb{E}_{(X,Y)\sim \mathbb{P}}[\ell(h(X),Y)]$，其中$L(h)$称为$h$的population risk(or expected loss)。函数集$\mathscr{H}$上最小的population risk是$\inf\limits_{g \in \mathscr{H}}L(g)$，所以我们把$E(h)=L(h)-\inf\limits_{g \in \mathscr{H}}L(g)$称为$h$的excess risk。
...
注意到，因为训练样本本身也是独立同分布的从ground truth采样的，所以empirical risk的期望就是population risk：$\mathbb{E}_{(x_i,y_i)\stackrel{\text{i.i.d.}}{\sim}\mathbb{P}}[\hat L_n(h)]=\mathbb{E}_{(x_i,y_i)\stackrel{\text{i.i.d.}}{\sim}\mathbb{P}}\left[\dfrac{1}{n}\sum\limits_{i=1}^{n}\ell(h(x_i),y_i)\right]=\mathbb{E}_{(X,Y)\sim \mathbb{P}}[\ell(h(X),Y)]=L(h)$，也即empirical risk的估计与ground truth相比是不带偏差(bias)的。
